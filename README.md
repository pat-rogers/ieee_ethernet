...
`test/host/` holds a partial host-native harness for the root package, which is target-independent and so runs on the development machine unchanged. Only the closure of the main is built, so the SMI and PHY units are not compiled there.
